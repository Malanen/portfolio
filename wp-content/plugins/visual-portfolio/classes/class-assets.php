...
class Visual_Portfolio_Assets {
    ...
    public static function enqueue_stored_assets( $type = 'script' ) {
        ...
        foreach ( self::$stored_assets[ $type ] as $name => $data ) {
            if ( isset( $data['value'] ) && $data['value'] ) {
                if ( 'script' === $type ) {
                    wp_enqueue_script( $name, '', array(), '2.5.0', true );
                } elseif ( is_string( $data['value'] ) ) {
                    visual_portfolio()->include_template_style( $name, $data['value'], array(), '2.5.0' );
                } else {
                    wp_enqueue_style( $name, '', array(), '2.5.0' );
                }
    ...
    public static function enqueue( $options ) {
        ...
        wp_register_style( $controls_css_handle, false, array(), '2.5.0' );
    ...
    public function register_scripts() {
        ...
        foreach ( $vp_styles as $name => $data ) {
            wp_register_style( $name, visual_portfolio()->plugin_url . $data[0], $data[1], '2.5.0' );
        ...
        foreach ( $vp_scripts as $name => $data ) {
            wp_register_script( $name, visual_portfolio()->plugin_url . $data[0], $data[1], '2.5.0', true );
        }
